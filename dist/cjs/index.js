...
const router_1 = require("@webcarrot/router");
...
const parsePath = (info) => {
    ...
    if (info instanceof Array) {
        ...
        const pathKeys = [];
        const pathRegExp = path_to_regexp_1.pathToRegexp(info, pathKeys);
        ...
        const pathKeys = [];
        const pathRegExp = path_to_regexp_1.pathToRegexp(info, pathKeys);
        const compiler = path_to_regexp_1.compile(info);
        match.push((url, _, { rootPath }) => matchByRegExp(url, pathKeys, pathRegExp, rootPath));
